...
public class datapokemon extends JFrame {
    ...
    public datapokemon() {
        setTitle("Pusat Pokémon - Data Pokémon");
        setSize(750, 480); 
        ...
        JLabel lblTitle = new JLabel("POKÉMON STORAGE SYSTEM", SwingConstants.CENTER);
        lblTitle.setFont(new Font("Monospaced", Font.BOLD, 20));
        lblTitle.setForeground(BLUE_SYS);
        contentPanel.add(lblTitle, BorderLayout.NORTH);

        model = new DefaultTableModel(
            new Object[]{"ID", "Nama Pokémon", "Tipe", "Level", "Trainer"}, 0) {
            @Override
        ...
        btnUbah.addActionListener(e -> {
            ...
            if (row == -1) {
                JOptionPane.showMessageDialog(this, "Pilih data pokémon terlebih dahulu.");
                return;
            }
...
private JButton createSysButton(String text, Color bg, Color fg) {
    ...
    private void hapusData() {
        ...
        String namaPokemon = model.getValueAt(row, 1).toString();
        int id = Integer.parseInt(model.getValueAt(row, 0).toString());

        int confirm = JOptionPane.showConfirmDialog(this,
            "Hapus pokémon \"" + namaPokemon + "\"?",
